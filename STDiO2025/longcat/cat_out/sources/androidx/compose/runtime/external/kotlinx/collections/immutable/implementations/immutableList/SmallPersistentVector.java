...
public final class SmallPersistentVector<E> extends AbstractPersistentList<E> implements ImmutableList<E> {
    ...
    public SmallPersistentVector(Object[] objArr) {
        this.buffer = objArr;
        CommonFunctionsKt.m3377assert(objArr.length <= 32);
    }
    ...
    public PersistentList<E> addAll(int index, Collection<? extends E> c2) {
        ListImplementation.checkPositionIndex$runtime_release(index, size());
        if (c2.size() + size() <= 32) {
            Object[] objArrBufferOfSize = bufferOfSize(c2.size() + size());
            ArraysKt___ArraysJvmKt.copyInto$default(this.buffer, objArrBufferOfSize, 0, 0, index, 6, (Object) null);
            ArraysKt.copyInto(this.buffer, objArrBufferOfSize, c2.size() + index, index, size());
