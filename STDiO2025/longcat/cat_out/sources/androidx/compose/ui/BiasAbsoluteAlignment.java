...
public final /* data */ class BiasAbsoluteAlignment implements Alignment {
    ...
    public long mo3397alignKFBX0sM(long size, long space, LayoutDirection layoutDirection) {
        long jIntSize = IntSizeKt.IntSize(IntSize.m6403getWidthimpl(space) - IntSize.m6403getWidthimpl(size), IntSize.m6402getHeightimpl(space) - IntSize.m6402getHeightimpl(size));
        float f2 = 1;
        return IntOffsetKt.IntOffset(Math.round((this.horizontalBias + f2) * (IntSize.m6403getWidthimpl(jIntSize) / 2.0f)), Math.round((f2 + this.verticalBias) * (IntSize.m6402getHeightimpl(jIntSize) / 2.0f)));
    }
