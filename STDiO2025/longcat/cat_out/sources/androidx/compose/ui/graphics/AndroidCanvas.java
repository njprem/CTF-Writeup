...
public final class AndroidCanvas implements Canvas {
    ...
    private final void drawLines(List<Offset> points, Paint paint, int stepBy) {
        if (points.size() >= 2) {
            android.graphics.Paint paintAsFrameworkPaint = paint.getInternalPaint();
            int i2 = 0;
            while (i2 < points.size() - 1) {
                long jM3561unboximpl = points.get(i2).getPackedValue();
                long jM3561unboximpl2 = points.get(i2 + 1).getPackedValue();
                this.internalCanvas.drawLine(Offset.m3551getXimpl(jM3561unboximpl), Offset.m3552getYimpl(jM3561unboximpl), Offset.m3551getXimpl(jM3561unboximpl2), Offset.m3552getYimpl(jM3561unboximpl2), paintAsFrameworkPaint);
                i2 += stepBy;
            }
        }
    }
    ...
    private final void drawPoints(List<Offset> points, Paint paint) {
        ...
        for (int i2 = 0; i2 < size; i2++) {
            long jM3561unboximpl = points.get(i2).getPackedValue();
            this.internalCanvas.drawPoint(Offset.m3551getXimpl(jM3561unboximpl), Offset.m3552getYimpl(jM3561unboximpl), paint.getInternalPaint());
        }
    }
    ...
    private final void drawRawLines(float[] points, Paint paint, int stepBy) {
        ...
        while (i2 < points.length - 3) {
            this.internalCanvas.drawLine(points[i2], points[i2 + 1], points[i2 + 2], points[i2 + 3], paintAsFrameworkPaint);
            i2 += stepBy * 2;
        }
    }
    ...
    private final void drawRawPoints(float[] points, Paint paint, int stepBy) {
        if (points.length % 2 == 0) {
            ...
            while (i2 < points.length - 1) {
                this.internalCanvas.drawPoint(points[i2], points[i2 + 1], paintAsFrameworkPaint);
                i2 += stepBy;
            }
        }
    }
    ...
    public void mo3645clipPathmtrdDE(Path path, int clipOp) {
        android.graphics.Canvas canvas = this.internalCanvas;
        if (!(path instanceof AndroidPath)) {
            throw new UnsupportedOperationException("Unable to obtain android.graphics.Path");
        }
        canvas.clipPath(((AndroidPath) path).getInternalPath(), m3655toRegionOp7u2Bmg(clipOp));
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: clipRect-N_I0leg, reason: not valid java name */
    public void mo3646clipRectN_I0leg(float left, float top, float right, float bottom, int clipOp) {
        this.internalCanvas.clipRect(left, top, right, bottom, m3655toRegionOp7u2Bmg(clipOp));
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: concat-58bKbWc, reason: not valid java name */
    public void mo3647concat58bKbWc(float[] matrix) {
        if (MatrixKt.m4046isIdentity58bKbWc(matrix)) {
            return;
        }
        android.graphics.Matrix matrix2 = new android.graphics.Matrix();
        AndroidMatrixConversions_androidKt.m3662setFromEL8BTi8(matrix2, matrix);
    ...
    public void mo3648drawCircle9KIMszo(long center, float radius, Paint paint) {
        this.internalCanvas.drawCircle(Offset.m3551getXimpl(center), Offset.m3552getYimpl(center), radius, paint.getInternalPaint());
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: drawImage-d-4ec7I, reason: not valid java name */
    public void mo3649drawImaged4ec7I(ImageBitmap image, long topLeftOffset, Paint paint) {
        this.internalCanvas.drawBitmap(AndroidImageBitmap_androidKt.asAndroidBitmap(image), Offset.m3551getXimpl(topLeftOffset), Offset.m3552getYimpl(topLeftOffset), paint.getInternalPaint());
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: drawImageRect-HPBpro0, reason: not valid java name */
    public void mo3650drawImageRectHPBpro0(ImageBitmap image, long srcOffset, long srcSize, long dstOffset, long dstSize, Paint paint) {
        ...
        rect.left = IntOffset.m6361getXimpl(srcOffset);
        rect.top = IntOffset.m6362getYimpl(srcOffset);
        rect.right = IntSize.m6403getWidthimpl(srcSize) + IntOffset.m6361getXimpl(srcOffset);
        rect.bottom = IntSize.m6402getHeightimpl(srcSize) + IntOffset.m6362getYimpl(srcOffset);
        Unit unit = Unit.INSTANCE;
        Rect rect2 = this.dstRect;
        Intrinsics.checkNotNull(rect2);
        rect2.left = IntOffset.m6361getXimpl(dstOffset);
        rect2.top = IntOffset.m6362getYimpl(dstOffset);
        rect2.right = IntSize.m6403getWidthimpl(dstSize) + IntOffset.m6361getXimpl(dstOffset);
        rect2.bottom = IntSize.m6402getHeightimpl(dstSize) + IntOffset.m6362getYimpl(dstOffset);
        canvas.drawBitmap(bitmapAsAndroidBitmap, rect, rect2, paint.getInternalPaint());
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: drawLine-Wko1d7g, reason: not valid java name */
    public void mo3651drawLineWko1d7g(long p1, long p2, Paint paint) {
        this.internalCanvas.drawLine(Offset.m3551getXimpl(p1), Offset.m3552getYimpl(p1), Offset.m3551getXimpl(p2), Offset.m3552getYimpl(p2), paint.getInternalPaint());
    }
    ...
    public void mo3652drawPointsO7TthRY(int pointMode, List<Offset> points, Paint paint) {
        PointMode.Companion companion = PointMode.INSTANCE;
        if (PointMode.m4092equalsimpl0(pointMode, companion.m4096getLinesr_lszbg())) {
            drawLines(points, paint, 2);
        } else if (PointMode.m4092equalsimpl0(pointMode, companion.m4098getPolygonr_lszbg())) {
            drawLines(points, paint, 1);
        } else if (PointMode.m4092equalsimpl0(pointMode, companion.m4097getPointsr_lszbg())) {
    ...
    public void mo3653drawRawPointsO7TthRY(int pointMode, float[] points, Paint paint) {
        if (points.length % 2 != 0) {
    ...
    public void mo3654drawVerticesTPEHhCM(Vertices vertices, int blendMode, Paint paint) {
        this.internalCanvas.drawVertices(AndroidVertexMode_androidKt.m3698toAndroidVertexModeJOOmi9M(vertices.getVertexMode()), vertices.getPositions().length, vertices.getPositions(), 0, vertices.getTextureCoordinates(), 0, vertices.getColors(), 0, vertices.getIndices(), 0, vertices.getIndices().length, paint.getInternalPaint());
    }
    ...
    public final Region.Op m3655toRegionOp7u2Bmg(int i2) {
        return ClipOp.m3776equalsimpl0(i2, ClipOp.INSTANCE.m3780getDifferencertfAjoo()) ? Region.Op.DIFFERENCE : Region.Op.INTERSECT;
    }
