...
public final class PathHitTester {
    ...
    public final boolean m4075containsk4lQ0M(long position) {
        int i2;
        int iLineWinding;
        if (!this.path.isEmpty() && this.bounds.m3577containsk4lQ0M(position)) {
            ...
            if (((IntervalTree) intervalTree).root != ((IntervalTree) intervalTree).terminator) {
                ...
                while (arrayList.size() > 0) {
                    ...
                    if (node.overlaps(fM3552getYimpl, fM3552getYimpl)) {
                        T data = node.getData();
                        ...
                        if (i3 == 1) {
                            iLineWinding = BezierKt.lineWinding(points, fM3551getXimpl, fM3552getYimpl);
                        } else if (i3 == 2) {
                            iLineWinding = BezierKt.quadraticWinding(points, fM3551getXimpl, fM3552getYimpl, fArr, fArr2);
                        } else if (i3 == 3) {
                            iLineWinding = BezierKt.cubicWinding(points, fM3551getXimpl, fM3552getYimpl, fArr, fArr2);
                        }
                        i2 += iLineWinding;
                    }
                    if (node.getLeft() != ((IntervalTree) intervalTree).terminator && node.getLeft().getMax() >= fM3552getYimpl) {
                        arrayList.add(node.getLeft());
                    }
                    if (node.getRight() != ((IntervalTree) intervalTree).terminator && node.getRight().getMin() <= fM3552getYimpl) {
            ...
            if (PathFillType.m4069equalsimpl0(this.path.mo3683getFillTypeRgk1Os(), PathFillType.INSTANCE.m4073getEvenOddRgk1Os())) {
                i2 &= 1;
            }
            if (i2 != 0) {
                return true;
