...
public final class MotionEventAdapter {
    ...
    private final PointerInputEventData createPointerInputEventData(PositionCalculator positionCalculator, MotionEvent motionEvent, int index, boolean pressed) {
        long jMo5073localToScreenMKHz9U;
        long jM4921getComposePointerId_I2yYro = m4921getComposePointerId_I2yYro(motionEvent.getPointerId(index));
        float pressure = motionEvent.getPressure(index);
        long jOffset = OffsetKt.Offset(motionEvent.getX(index), motionEvent.getY(index));
        long jM3545copydBAh8RU$default = Offset.m3545copydBAh8RU$default(jOffset, 0.0f, 0.0f, 3, null);
        if (index == 0) {
            jMo5073localToScreenMKHz9U = OffsetKt.Offset(motionEvent.getRawX(), motionEvent.getRawY());
            jOffset = positionCalculator.mo5074screenToLocalMKHz9U(jMo5073localToScreenMKHz9U);
        } else if (Build.VERSION.SDK_INT >= 29) {
            jMo5073localToScreenMKHz9U = MotionEventHelper.INSTANCE.m4922toRawOffsetdBAh8RU(motionEvent, index);
            jOffset = positionCalculator.mo5074screenToLocalMKHz9U(jMo5073localToScreenMKHz9U);
        } else {
            jMo5073localToScreenMKHz9U = positionCalculator.mo5073localToScreenMKHz9U(jOffset);
        }
        long j2 = jMo5073localToScreenMKHz9U;
        long j3 = jOffset;
        int toolType = motionEvent.getToolType(index);
        int iM5071getUnknownT8wyACA = toolType != 0 ? toolType != 1 ? toolType != 2 ? toolType != 3 ? toolType != 4 ? PointerType.INSTANCE.m5071getUnknownT8wyACA() : PointerType.INSTANCE.m5067getEraserT8wyACA() : PointerType.INSTANCE.m5068getMouseT8wyACA() : PointerType.INSTANCE.m5069getStylusT8wyACA() : PointerType.INSTANCE.m5070getTouchT8wyACA() : PointerType.INSTANCE.m5071getUnknownT8wyACA();
        ...
        return new PointerInputEventData(jM4921getComposePointerId_I2yYro, motionEvent.getEventTime(), j2, j3, pressed, pressure, iM5071getUnknownT8wyACA, this.activeHoverIds.get(motionEvent.getPointerId(index), false), arrayList, motionEvent.getActionMasked() == 8 ? OffsetKt.Offset(motionEvent.getAxisValue(10), (-motionEvent.getAxisValue(9)) + 0.0f) : Offset.INSTANCE.m3567getZeroF1C5BW0(), jM3545copydBAh8RU$default, null);
    }

    /* renamed from: getComposePointerId-_I2yYro, reason: not valid java name */
    private final long m4921getComposePointerId_I2yYro(int motionEventPointerId) {
        long jValueAt;
        ...
        return PointerId.m4971constructorimpl(jValueAt);
    }
