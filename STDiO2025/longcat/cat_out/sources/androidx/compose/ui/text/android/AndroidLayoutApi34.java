...
public final class AndroidLayoutApi34 {
    ...
    public final int[] getRangeForRect$ui_text_release(TextLayout layout, RectF rectF, int granularity, final Function2<? super RectF, ? super RectF, Boolean> inclusionStrategy) {
        SegmentFinder segmentFinderJ;
        if (granularity == 1) {
            segmentFinderJ = Api34SegmentFinder.INSTANCE.toAndroidSegmentFinder$ui_text_release(new WordSegmentFinder(layout.getText(), layout.getWordIterator()));
        } else {
            f.p();
            segmentFinderJ = f.j(f.i(layout.getText(), layout.getTextPaint()));
        }
        return layout.getLayout().getRangeForRect(rectF, segmentFinderJ, new Layout.TextInclusionStrategy() { // from class: androidx.compose.ui.text.android.a
