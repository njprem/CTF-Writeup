...
public final class BringIntoViewRequestPriorityQueue {
    ...
    public final void cancelAndRemoveAll(Throwable cause) {
        ...
        for (int i3 = 0; i3 < size; i3++) {
            cancellableContinuationArr[i3].cancel(cause);
        }
    ...
    public final boolean enqueue(final ContentInViewNode.Request request) {
        ...
        if (rectInvoke == null) {
            ...
            continuation.resumeWith(Result.m6498constructorimpl(Unit.INSTANCE));
            return false;
        }
    ...
    public final void forEachFromSmallest(Function1<? super Rect, Unit> block) {
        ...
        if (size > 0) {
            ...
            do {
                block.invoke(((ContentInViewNode.Request) content[i2]).getCurrentBounds().invoke());
                i2--;
            } while (i2 >= 0);
        }
    }
    ...
    public final void resumeAndRemoveAll() {
        ...
        if (first <= last) {
            while (true) {
                this.requests.getContent()[first].getContinuation().resumeWith(Result.m6498constructorimpl(Unit.INSTANCE));
                if (first == last) {
                    break;
                } else {
                    first++;
    ...
    public final void resumeAndRemoveWhile(Function1<? super Rect, Boolean> block) {
        while (this.requests.isNotEmpty() && block.invoke(((ContentInViewNode.Request) this.requests.last()).getCurrentBounds().invoke()).booleanValue()) {
            ((ContentInViewNode.Request) this.requests.removeAt(this.requests.getSize() - 1)).getContinuation().resumeWith(Result.m6498constructorimpl(Unit.INSTANCE));
        }
    }
}
