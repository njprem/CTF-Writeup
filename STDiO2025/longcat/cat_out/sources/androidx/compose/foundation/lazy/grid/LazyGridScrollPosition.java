...
public final class LazyGridScrollPosition {
    ...
    private final void update(int index, int scrollOffset) {
        if (index < 0.0f) {
            throw new IllegalArgumentException(d.m("Index should be non-negative (", index, ')').toString());
        }
        setIndex(index);
    ...
    public final void updateFromMeasureResult(LazyGridMeasureResult measureResult) {
        ...
        if (this.hadFirstNotEmptyLayout || measureResult.getTotalItemsCount() > 0) {
            ...
            if (firstVisibleLineScrollOffset < 0.0f) {
                throw new IllegalStateException(d.m("scrollOffset should be non-negative (", firstVisibleLineScrollOffset, ')').toString());
            }
    ...
    public final void updateScrollOffset(int scrollOffset) {
        if (scrollOffset < 0.0f) {
            throw new IllegalStateException(d.m("scrollOffset should be non-negative (", scrollOffset, ')').toString());
        }
        setScrollOffset(scrollOffset);
    }
    ...
    public LazyGridScrollPosition(int i2, int i3) {
        this.index = SnapshotIntStateKt.mutableIntStateOf(i2);
        this.scrollOffset = SnapshotIntStateKt.mutableIntStateOf(i3);
        this.nearestRangeState = new LazyLayoutNearestRangeState(i2, 90, ComposerKt.invocationKey);
    }
