...
public final class LazyGridSpanLayoutProvider {
    ...
    private final List<GridItemSpan> getDefaultSpans(int currentSlotsPerLine) {
        ...
        for (int i2 = 0; i2 < currentSlotsPerLine; i2++) {
            arrayList.add(GridItemSpan.m761boximpl(LazyGridSpanKt.GridItemSpan(1)));
        }
    ...
    */
    ...
    public final int getLineIndexOfItem(final int itemIndex) {
        ...
        while (true) {
            ...
            if (bucketSize % getBucketSize() == 0 && bucketSize / getBucketSize() >= this.buckets.size()) {
                this.buckets.add(new Bucket(i5 - (i4 <= 0 ? 0 : 1), i2, i3, null));
            }
            firstItemIndex = i5;
        }
        return spanOf(itemIndex, this.slotsPerLine - i4) + i4 > this.slotsPerLine ? bucketSize + 1 : bucketSize;
    }
    ...
    public final int spanOf(int itemIndex, int maxSpan) {
        ...
        return GridItemSpan.m765getCurrentLineSpanimpl(interval.getValue().getSpan().invoke(lazyGridItemSpanScopeImpl, Integer.valueOf(itemIndex - interval.getStartIndex())).getPackedValue());
    }
