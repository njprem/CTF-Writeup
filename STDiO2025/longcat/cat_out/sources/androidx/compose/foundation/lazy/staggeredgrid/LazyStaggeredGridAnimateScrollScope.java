...
public final class LazyStaggeredGridAnimateScrollScope implements LazyLayoutAnimateScrollScope {
    ...
    private final int calculateVisibleItemsAverageSize(LazyStaggeredGridLayoutInfo layoutInfo) {
        ...
        int iM6402getHeightimpl = 0;
        for (int i2 = 0; i2 < size; i2++) {
            LazyStaggeredGridItemInfo lazyStaggeredGridItemInfo = visibleItemsInfo.get(i2);
            iM6402getHeightimpl += layoutInfo.getOrientation() == Orientation.Vertical ? IntSize.m6402getHeightimpl(lazyStaggeredGridItemInfo.getSize()) : IntSize.m6403getWidthimpl(lazyStaggeredGridItemInfo.getSize());
        }
        return layoutInfo.getMainAxisItemSpacing() + (iM6402getHeightimpl / visibleItemsInfo.size());
    }
    ...
    public float calculateDistanceTo(int targetIndex) {
        ...
        if (lazyStaggeredGridItemInfo != null) {
            return layoutInfo.getOrientation() == Orientation.Vertical ? IntOffset.m6362getYimpl(r4.getOffset()) : IntOffset.m6361getXimpl(r4.getOffset());
        }
