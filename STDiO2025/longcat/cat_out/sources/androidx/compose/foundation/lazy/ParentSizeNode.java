...
final class ParentSizeNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public /* synthetic */ ParentSizeNode(float f2, State state, State state2, int i2, DefaultConstructorMarker defaultConstructorMarker) {
    ...
    public MeasureResult mo54measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j2) {
        ...
        int iM6188getMinWidthimpl = iRound != Integer.MAX_VALUE ? iRound : Constraints.m6188getMinWidthimpl(j2);
        int iM6187getMinHeightimpl = iRound2 != Integer.MAX_VALUE ? iRound2 : Constraints.m6187getMinHeightimpl(j2);
        if (iRound == Integer.MAX_VALUE) {
            iRound = Constraints.m6186getMaxWidthimpl(j2);
        }
        if (iRound2 == Integer.MAX_VALUE) {
            iRound2 = Constraints.m6185getMaxHeightimpl(j2);
        }
        final Placeable placeableMo5120measureBRTryo0 = measurable.mo5120measureBRTryo0(ConstraintsKt.Constraints(iM6188getMinWidthimpl, iRound, iM6187getMinHeightimpl, iRound2));
        return MeasureScope.layout$default(measureScope, placeableMo5120measureBRTryo0.getWidth(), placeableMo5120measureBRTryo0.getHeight(), null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.lazy.ParentSizeNode$measure$1
            {
                super(1);
            }
