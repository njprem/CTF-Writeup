...
final class IntrinsicHeightNode extends IntrinsicSizeModifier {
    ...
    public long mo596calculateContentConstraintsl58MMJ0(MeasureScope measureScope, Measurable measurable, long j2) {
        int iMinIntrinsicHeight = this.height == IntrinsicSize.Min ? measurable.minIntrinsicHeight(Constraints.m6186getMaxWidthimpl(j2)) : measurable.maxIntrinsicHeight(Constraints.m6186getMaxWidthimpl(j2));
        if (iMinIntrinsicHeight < 0) {
            iMinIntrinsicHeight = 0;
        }
        return Constraints.INSTANCE.m6197fixedHeightOenEA2s(iMinIntrinsicHeight);
    }
