...
public final class TextFieldBuffer implements Appendable {
    ...
    public interface ChangeList {
        ...
        long mo1065getOriginalRangejx7JFs(int changeIndex);

        /* renamed from: getRange--jx7JFs, reason: not valid java name */
        long mo1066getRangejx7JFs(int changeIndex);
    }
    ...
    private final void onTextWillChange(int replaceStart, int replaceEnd, int newLength) {
        ...
        int iM5719getMinimpl = TextRange.m5719getMinimpl(getSelectionInChars());
        int iM5718getMaximpl = TextRange.m5718getMaximpl(getSelectionInChars());
        if (iM5718getMaximpl < iMin) {
            return;
        }
        if (iM5719getMinimpl <= iMin && iMax <= iM5718getMaximpl) {
            i2 = newLength - (iMax - iMin);
            if (iM5719getMinimpl == iM5718getMaximpl) {
            }
            iMin = iM5718getMaximpl + i2;
            this.selectionInChars = TextRangeKt.TextRange(iM5719getMinimpl, iMin);
        }
        if (iM5719getMinimpl > iMin && iM5718getMaximpl < iMax) {
            iMin += newLength;
            iM5719getMinimpl = iMin;
        } else if (iM5719getMinimpl >= iMax) {
            i2 = newLength - (iMax - iMin);
        } else if (iMin < iM5719getMinimpl) {
            iM5719getMinimpl = iMin + newLength;
            iMin = (newLength - (iMax - iMin)) + iM5718getMaximpl;
        }
        this.selectionInChars = TextRangeKt.TextRange(iM5719getMinimpl, iMin);
        iM5719getMinimpl += i2;
        iMin = iM5718getMaximpl + i2;
    ...
    private final void requireValidIndex(int index, boolean startExclusive, boolean endExclusive) {
        int i2 = startExclusive ? 0 : -1;
        int length = endExclusive ? getLength() : getLength() + 1;
        if (i2 > index || index >= length) {
            throw new IllegalArgumentException(a.o(d.q("Expected ", index, i2, " to be in [", ", "), length, ')').toString());
        }
    }

    /* renamed from: requireValidRange-5zc-tL8, reason: not valid java name */
    private final void m1059requireValidRange5zctL8(long range) {
        long jTextRange = TextRangeKt.TextRange(0, getLength());
        if (TextRange.m5711contains5zctL8(jTextRange, range)) {
            return;
        }
        throw new IllegalArgumentException(("Expected " + ((Object) TextRange.m5724toStringimpl(range)) + " to be in " + ((Object) TextRange.m5724toStringimpl(jTextRange))).toString());
    }

    /* renamed from: toTextFieldCharSequence-udt6zUU$foundation_release$default, reason: not valid java name */
    public static /* synthetic */ TextFieldCharSequence m1060toTextFieldCharSequenceudt6zUU$foundation_release$default(TextFieldBuffer textFieldBuffer, long j2, TextRange textRange, int i2, Object obj) {
        if ((i2 & 1) != 0) {
        ...
        return textFieldBuffer.m1064toTextFieldCharSequenceudt6zUU$foundation_release(j2, textRange);
    }
    ...
    public final long m1061getOriginalSelectiond9O1mEE() {
    ...
    public final boolean hasSelection() {
        return !TextRange.m5715getCollapsedimpl(getSelectionInChars());
    }
    ...
    public final void replace$foundation_release(int start, int end, CharSequence text, int textStart, int textEnd) {
        if (start > end) {
            throw new IllegalArgumentException(d.n("Expected start=", start, end, " <= end=").toString());
        }
        if (textStart > textEnd) {
            throw new IllegalArgumentException(d.n("Expected textStart=", textStart, textEnd, " <= textEnd=").toString());
        }
    ...
    public final void revertAllChanges() {
        ...
        m1063setSelection5zctL8(this.originalValue.getSelection());
        clearChangeList();
    }

    /* renamed from: setSelection-5zc-tL8, reason: not valid java name */
    public final void m1063setSelection5zctL8(long j2) {
        m1059requireValidRange5zctL8(j2);
    ...
    public final void setTextIfChanged$foundation_release(CharSequence newText) {
        ...
        if (partialGapBuffer.length() <= 0 || newText.length() <= 0) {
            ...
            while (true) {
                if (!z) {
                    if (partialGapBuffer.charAt(i4) == newText.charAt(i5)) {
                        i4++;
                        i5++;
                    } else {
                        z = true;
        ...
        int i7 = length2;
        if (i2 < i6 || i3 < i7) {
            replace$foundation_release(i2, i6, newText, i3, i7);
        }
    }
    ...
    public final TextFieldCharSequence m1064toTextFieldCharSequenceudt6zUU$foundation_release(long selection, TextRange composition) {
