...
public final class TextLayoutHelperKt {
    ...
    public static final boolean m1048canReuse7_7YC6M(TextLayoutResult textLayoutResult, AnnotatedString annotatedString, TextStyle textStyle, List<AnnotatedString.Range<Placeholder>> list, int i2, boolean z, int i3, Density density, LayoutDirection layoutDirection, FontFamily.Resolver resolver, long j2) {
        TextLayoutInput layoutInput = textLayoutResult.getLayoutInput();
        if (textLayoutResult.getMultiParagraph().getIntrinsics().getHasStaleResolvedFonts() || !Intrinsics.areEqual(layoutInput.getText(), annotatedString) || !layoutInput.getStyle().hasSameLayoutAffectingAttributes(textStyle) || !Intrinsics.areEqual(layoutInput.getPlaceholders(), list) || layoutInput.getMaxLines() != i2 || layoutInput.getSoftWrap() != z || !TextOverflow.m6164equalsimpl0(layoutInput.getOverflow(), i3) || !Intrinsics.areEqual(layoutInput.getDensity(), density) || layoutInput.getLayoutDirection() != layoutDirection || !Intrinsics.areEqual(layoutInput.getFontFamilyResolver(), resolver) || Constraints.m6188getMinWidthimpl(j2) != Constraints.m6188getMinWidthimpl(layoutInput.getConstraints())) {
            return false;
        }
        if (z || TextOverflow.m6164equalsimpl0(i3, TextOverflow.INSTANCE.m6172getEllipsisgIe3tQ8())) {
            return Constraints.m6186getMaxWidthimpl(j2) == Constraints.m6186getMaxWidthimpl(layoutInput.getConstraints()) && Constraints.m6185getMaxHeightimpl(j2) == Constraints.m6185getMaxHeightimpl(layoutInput.getConstraints());
        }
        return true;
    }
}
