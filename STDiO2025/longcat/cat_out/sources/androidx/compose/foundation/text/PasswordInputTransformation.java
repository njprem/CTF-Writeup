...
public final class PasswordInputTransformation implements InputTransformation {
    ...
    private final void setRevealCodepointIndex(int i2) {
        this.revealCodepointIndex.setIntValue(i2);
    }
    ...
    public void transformInput(TextFieldBuffer textFieldBuffer) {
        if (textFieldBuffer.getChanges().getChangeCount() != 1 || TextRange.m5717getLengthimpl(textFieldBuffer.getChanges().mo1066getRangejx7JFs(0)) != 1 || TextRange.m5717getLengthimpl(textFieldBuffer.getChanges().mo1065getOriginalRangejx7JFs(0)) != 0 || textFieldBuffer.hasSelection()) {
            setRevealCodepointIndex(-1);
            return;
        }
        int iM5719getMinimpl = TextRange.m5719getMinimpl(textFieldBuffer.getChanges().mo1066getRangejx7JFs(0));
        if (getRevealCodepointIndex$foundation_release() != iM5719getMinimpl) {
            this.scheduleHide.invoke();
            setRevealCodepointIndex(iM5719getMinimpl);
        }
    }
}
