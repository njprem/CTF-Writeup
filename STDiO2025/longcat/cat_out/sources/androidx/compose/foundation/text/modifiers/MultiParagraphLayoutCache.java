...
public final class MultiParagraphLayoutCache {
    ...
    private final MultiParagraph m1260layoutTextK40F9xA(long constraints, LayoutDirection layoutDirection) {
        MultiParagraphIntrinsics layoutDirection2 = setLayoutDirection(layoutDirection);
        return new MultiParagraph(layoutDirection2, LayoutUtilsKt.m1255finalConstraintstfFHcEY(constraints, this.softWrap, this.overflow, layoutDirection2.getMaxIntrinsicWidth()), LayoutUtilsKt.m1256finalMaxLinesxdlQI24(this.softWrap, this.overflow, this.maxLines), TextOverflow.m6164equalsimpl0(this.overflow, TextOverflow.INSTANCE.m6172getEllipsisgIe3tQ8()), null);
    }
    ...
    private final int m1261maxWidthBRTryo0(long constraints) {
        boolean z = this.softWrap;
        int i2 = this.overflow;
        MultiParagraphIntrinsics multiParagraphIntrinsics = this.paragraphIntrinsics;
        Intrinsics.checkNotNull(multiParagraphIntrinsics);
        return LayoutUtilsKt.m1257finalMaxWidthtfFHcEY(constraints, z, i2, multiParagraphIntrinsics.getMaxIntrinsicWidth());
    }

    /* renamed from: newLayoutWillBeDifferent-VKLhPVY, reason: not valid java name */
    private final boolean m1262newLayoutWillBeDifferentVKLhPVY(TextLayoutResult textLayoutResult, long j2, LayoutDirection layoutDirection) {
        ...
        if (Constraints.m6179equalsimpl0(j2, textLayoutResult.getLayoutInput().getConstraints())) {
            return false;
        }
        return Constraints.m6186getMaxWidthimpl(j2) != Constraints.m6186getMaxWidthimpl(textLayoutResult.getLayoutInput().getConstraints()) || ((float) Constraints.m6185getMaxHeightimpl(j2)) < textLayoutResult.getMultiParagraph().getHeight() || textLayoutResult.getMultiParagraph().getDidExceedMaxLines();
    }
    ...
    private final TextLayoutResult m1263textLayoutResultVKLhPVY(LayoutDirection layoutDirection, long finalConstraints, MultiParagraph multiParagraph) {
        ...
        Density density = this.density;
        Intrinsics.checkNotNull(density);
        return new TextLayoutResult(new TextLayoutInput(annotatedString, textStyle, listEmptyList, i2, z, i3, density, layoutDirection, this.fontFamilyResolver, finalConstraints, (DefaultConstructorMarker) null), multiParagraph, ConstraintsKt.m6200constrain4WqzIAM(finalConstraints, IntSizeKt.IntSize(TextDelegateKt.ceilToIntPx(fMin), TextDelegateKt.ceilToIntPx(multiParagraph.getHeight()))), null);
    }
    ...
    public final int intrinsicHeight(int width, LayoutDirection layoutDirection) {
        ...
        int iCeilToIntPx = TextDelegateKt.ceilToIntPx(m1260layoutTextK40F9xA(ConstraintsKt.Constraints(0, width, 0, Integer.MAX_VALUE), layoutDirection).getHeight());
    ...
    public final boolean m1264layoutWithConstraintsK40F9xA(long constraints, LayoutDirection layoutDirection) {
        ...
        if (m1262newLayoutWillBeDifferentVKLhPVY(this.layoutCache, constraints, layoutDirection2)) {
            this.layoutCache = m1263textLayoutResultVKLhPVY(layoutDirection2, constraints, m1260layoutTextK40F9xA(constraints, layoutDirection2));
            return true;
        }
        TextLayoutResult textLayoutResult = this.layoutCache;
        Intrinsics.checkNotNull(textLayoutResult);
        if (Constraints.m6179equalsimpl0(constraints, textLayoutResult.getLayoutInput().getConstraints())) {
            return false;
        }
        TextLayoutResult textLayoutResult2 = this.layoutCache;
        Intrinsics.checkNotNull(textLayoutResult2);
        this.layoutCache = m1263textLayoutResultVKLhPVY(layoutDirection2, constraints, textLayoutResult2.getMultiParagraph());
        return true;
    }
    ...
    public final void m1265updateZNqEYIc(AnnotatedString text, TextStyle style, FontFamily.Resolver fontFamilyResolver, int overflow, boolean softWrap, int maxLines, int minLines, List<AnnotatedString.Range<Placeholder>> placeholders) {
        this.text = text;
        this.style = style;
    ...
    public /* synthetic */ MultiParagraphLayoutCache(AnnotatedString annotatedString, TextStyle textStyle, FontFamily.Resolver resolver, int i2, boolean z, int i3, int i4, List list, int i5, DefaultConstructorMarker defaultConstructorMarker) {
        this(annotatedString, textStyle, resolver, (i5 & 8) != 0 ? TextOverflow.INSTANCE.m6171getClipgIe3tQ8() : i2, (i5 & 16) != 0 ? true : z, (i5 & 32) != 0 ? Integer.MAX_VALUE : i3, (i5 & 64) != 0 ? 1 : i4, (i5 & 128) != 0 ? null : list, null);
    }
}
