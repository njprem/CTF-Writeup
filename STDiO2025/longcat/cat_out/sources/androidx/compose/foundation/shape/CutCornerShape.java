...
public final class CutCornerShape extends CornerBasedShape {
    ...
    public Outline mo908createOutlineLjSzlW0(long size, float topStart, float topEnd, float bottomEnd, float bottomStart, LayoutDirection layoutDirection) {
        if (topStart + topEnd + bottomStart + bottomEnd == 0.0f) {
            return new Outline.Rectangle(SizeKt.m3641toRectuvyYCjk(size));
        }
        Path Path = AndroidPath_androidKt.Path();
        LayoutDirection layoutDirection2 = LayoutDirection.Ltr;
        float f2 = layoutDirection == layoutDirection2 ? topStart : topEnd;
        Path.moveTo(0.0f, f2);
        Path.lineTo(f2, 0.0f);
        if (layoutDirection == layoutDirection2) {
            topStart = topEnd;
        }
        Path.lineTo(Size.m3620getWidthimpl(size) - topStart, 0.0f);
        Path.lineTo(Size.m3620getWidthimpl(size), topStart);
        float f3 = layoutDirection == layoutDirection2 ? bottomEnd : bottomStart;
        Path.lineTo(Size.m3620getWidthimpl(size), Size.m3617getHeightimpl(size) - f3);
        Path.lineTo(Size.m3620getWidthimpl(size) - f3, Size.m3617getHeightimpl(size));
        if (layoutDirection == layoutDirection2) {
            bottomEnd = bottomStart;
        }
        Path.lineTo(bottomEnd, Size.m3617getHeightimpl(size));
        Path.lineTo(0.0f, Size.m3617getHeightimpl(size) - bottomEnd);
        Path.close();
