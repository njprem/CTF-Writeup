...
public final class PlatformMagnifierFactoryApi29Impl implements PlatformMagnifierFactory {
    ...
    public static final class PlatformMagnifierImpl extends PlatformMagnifierFactoryApi28Impl.PlatformMagnifierImpl {
        ...
        public void mo287updateWko1d7g(long sourceCenter, long magnifierCenter, float zoom) {
            if (!Float.isNaN(zoom)) {
                getMagnifier().setZoom(zoom);
            }
            if (OffsetKt.m3570isSpecifiedk4lQ0M(magnifierCenter)) {
                getMagnifier().show(Offset.m3551getXimpl(sourceCenter), Offset.m3552getYimpl(sourceCenter), Offset.m3551getXimpl(magnifierCenter), Offset.m3552getYimpl(magnifierCenter));
            } else {
                getMagnifier().show(Offset.m3551getXimpl(sourceCenter), Offset.m3552getYimpl(sourceCenter));
            }
        }
    }
    ...
    public PlatformMagnifierImpl mo288createnHHXs2Y(View view, boolean useTextDefault, long size, float cornerRadius, float elevation, boolean clippingEnabled, Density density, float initialZoom) {
        if (useTextDefault) {
            return new PlatformMagnifierImpl(new Magnifier(view));
        }
        long jMo335toSizeXkaWNTQ = density.mo335toSizeXkaWNTQ(size);
        float fMo334toPx0680j_4 = density.mo334toPx0680j_4(cornerRadius);
        float fMo334toPx0680j_42 = density.mo334toPx0680j_4(elevation);
        Magnifier.Builder builder = new Magnifier.Builder(view);
        if (jMo335toSizeXkaWNTQ != InlineClassHelperKt.UnspecifiedPackedFloats) {
            builder.setSize(MathKt.roundToInt(Size.m3620getWidthimpl(jMo335toSizeXkaWNTQ)), MathKt.roundToInt(Size.m3617getHeightimpl(jMo335toSizeXkaWNTQ)));
        }
        if (!Float.isNaN(fMo334toPx0680j_4)) {
            builder.setCornerRadius(fMo334toPx0680j_4);
        }
        if (!Float.isNaN(fMo334toPx0680j_42)) {
            builder.setElevation(fMo334toPx0680j_42);
        }
