...
public final class FlingCalculator {
    ...
    public static final /* data */ class FlingInfo {
        ...
        public int hashCode() {
            return Long.hashCode(this.duration) + d.b(Float.hashCode(this.initialVelocity) * 31, 31, this.distance);
        }

        public final float position(long time) {
            long j2 = this.duration;
            return AndroidFlingSpline.INSTANCE.flingPosition(j2 > 0 ? time / j2 : 1.0f).getDistanceCoefficient() * Math.signum(this.initialVelocity) * this.distance;
        }
        ...
        public final float velocity(long time) {
            long j2 = this.duration;
            return (((Math.signum(this.initialVelocity) * AndroidFlingSpline.INSTANCE.flingPosition(j2 > 0 ? time / j2 : 1.0f).getVelocityCoefficient()) * this.distance) / this.duration) * 1000.0f;
        }
    }
    ...
    public final float flingDistance(float velocity) {
        return (float) (Math.exp((FlingCalculatorKt.DecelerationRate / (FlingCalculatorKt.DecelerationRate - 1.0d)) * getSplineDeceleration(velocity)) * this.friction * this.magicPhysicalCoefficient);
    }
    ...
    public final FlingInfo flingInfo(float velocity) {
        ...
        double d2 = FlingCalculatorKt.DecelerationRate - 1.0d;
        return new FlingInfo(velocity, (float) (Math.exp((FlingCalculatorKt.DecelerationRate / d2) * splineDeceleration) * this.friction * this.magicPhysicalCoefficient), (long) (Math.exp(splineDeceleration / d2) * 1000.0d));
    }
