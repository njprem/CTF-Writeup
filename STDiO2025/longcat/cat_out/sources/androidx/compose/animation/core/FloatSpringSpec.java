...
public final class FloatSpringSpec implements FloatAnimationSpec {
    ...
    public long getDurationNanos(float initialValue, float targetValue, float initialVelocity) {
        ...
        return SpringEstimationKt.estimateAnimationDurationMillis(stiffness, dampingRatio, initialVelocity / f3, f2 / f3, 1.0f) * AnimationKt.MillisToNanos;
    }
    ...
    public float getValueFromNanos(long playTimeNanos, float initialValue, float targetValue, float initialVelocity) {
        long j2 = playTimeNanos / AnimationKt.MillisToNanos;
        this.spring.setFinalPosition(targetValue);
        return Motion.m125getValueimpl(this.spring.m131updateValuesIJZedt4$animation_core_release(initialValue, initialVelocity, j2));
    }
    ...
    public float getVelocityFromNanos(long playTimeNanos, float initialValue, float targetValue, float initialVelocity) {
        long j2 = playTimeNanos / AnimationKt.MillisToNanos;
        this.spring.setFinalPosition(targetValue);
        return Motion.m126getVelocityimpl(this.spring.m131updateValuesIJZedt4$animation_core_release(initialValue, initialVelocity, j2));
    }
