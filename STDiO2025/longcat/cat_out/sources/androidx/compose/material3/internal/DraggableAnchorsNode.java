...
final class DraggableAnchorsNode<T> extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo54measure3p2s80s(final MeasureScope measureScope, Measurable measurable, long j2) {
        ...
        if (!measureScope.isLookingAhead() || !this.didLookahead) {
            Pair<? extends DraggableAnchors<T>, ? extends T> pairInvoke = this.anchors.invoke(IntSize.m6395boximpl(IntSizeKt.IntSize(placeableMo5120measureBRTryo0.getWidth(), placeableMo5120measureBRTryo0.getHeight())), Constraints.m6174boximpl(j2));
        ...
        return MeasureScope.layout$default(measureScope, placeableMo5120measureBRTryo0.getWidth(), placeableMo5120measureBRTryo0.getHeight(), null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.material3.internal.DraggableAnchorsNode$measure$1
            ...
            public final void invoke2(Placeable.PlacementScope placementScope) {
                float fPositionOf = measureScope.isLookingAhead() ? this.getState().getAnchors().positionOf(this.getState().getTargetValue()) : this.getState().requireOffset();
                float f2 = this.getOrientation() == Orientation.Horizontal ? fPositionOf : 0.0f;
                if (this.getOrientation() != Orientation.Vertical) {
                    fPositionOf = 0.0f;
                }
                Placeable.PlacementScope.place$default(placementScope, placeableMo5120measureBRTryo0, MathKt.roundToInt(f2), MathKt.roundToInt(fPositionOf), 0.0f, 4, null);
            }
        }, 4, null);
    }
