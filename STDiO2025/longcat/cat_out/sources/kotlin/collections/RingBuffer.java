...
final class RingBuffer<T> extends AbstractList<T> implements RandomAccess {
    ...
    public RingBuffer(Object[] buffer, int i2) {
        ...
        this.buffer = buffer;
        if (i2 < 0) {
            throw new IllegalArgumentException(a.n("ring buffer filled size should not be negative but it is ", i2).toString());
        }
        if (i2 <= buffer.length) {
            this.capacity = buffer.length;
            this.size = i2;
        } else {
            StringBuilder sbP = a.p(i2, "ring buffer filled size: ", " cannot be larger than the buffer size: ");
            sbP.append(buffer.length);
    ...
    public final void removeFirst(int n2) {
        if (n2 < 0) {
            throw new IllegalArgumentException(a.n("n shouldn't be negative but it is ", n2).toString());
        }
        if (n2 > size()) {
            StringBuilder sbP = a.p(n2, "n shouldn't be greater than the buffer size: n = ", ", size = ");
            sbP.append(size());
            throw new IllegalArgumentException(sbP.toString().toString());
        }
        if (n2 > 0) {
