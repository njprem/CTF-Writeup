...
public abstract class Random {
    ...
    public byte[] nextBytes(byte[] array, int fromIndex, int toIndex) {
        ...
        if (fromIndex < 0 || fromIndex > array.length || toIndex < 0 || toIndex > array.length) {
            throw new IllegalArgumentException(a.o(d.q("fromIndex (", fromIndex, toIndex, ") or toIndex (", ") are out of range: 0.."), array.length, '.').toString());
        }
        ...
        for (int i5 = 0; i5 < i4; i5++) {
            array[fromIndex + i5] = (byte) (iNextBits >>> (i5 * 8));
        }
        return array;
    }
    ...
    public double nextDouble(double from, double until) {
        ...
        if (Double.isInfinite(d2) && !Double.isInfinite(from) && !Double.isNaN(from) && !Double.isInfinite(until) && !Double.isNaN(until)) {
            double d3 = 2;
            double dNextDouble2 = ((until / d3) - (from / d3)) * nextDouble();
    ...
    public int nextInt(int from, int until) {
        ...
        if (i3 > 0 || i3 == Integer.MIN_VALUE) {
            if (((-i3) & i3) == i3) {
                iNextBits = nextBits(RandomKt.fastLog2(i3));
            } else {
                do {
                    iNextInt = nextInt() >>> 1;
                    i2 = iNextInt % i3;
                } while ((i3 - 1) + (iNextInt - i2) < 0);
                iNextBits = i2;
            }
        ...
        if (j3 > 0) {
            if (((-j3) & j3) == j3) {
                ...
                if (i2 != 0) {
                    ...
                    jNextBits = (nextBits(RandomKt.fastLog2(i3)) << 32) + (nextInt() & 4294967295L);
                }
