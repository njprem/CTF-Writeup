...
public final class DispatchedTaskKt {
    ...
    public static final <T> void dispatch(DispatchedTask<? super T> dispatchedTask, int i2) {
        ...
        boolean z = i2 == 4;
        if (z || !(delegate$kotlinx_coroutines_core instanceof DispatchedContinuation) || isCancellableMode(i2) != isCancellableMode(dispatchedTask.resumeMode)) {
        ...
        if (coroutineDispatcher.isDispatchNeeded(context)) {
            coroutineDispatcher.mo8053dispatch(context, dispatchedTask);
        } else {
    ...
    public static final <T> void resume(DispatchedTask<? super T> dispatchedTask, Continuation<? super T> continuation, boolean z) {
        ...
        Object objM6498constructorimpl = Result.m6498constructorimpl(successfulResult$kotlinx_coroutines_core);
        if (!z) {
            continuation.resumeWith(objM6498constructorimpl);
            return;
        }
        ...
        try {
            dispatchedContinuation.continuation.resumeWith(objM6498constructorimpl);
